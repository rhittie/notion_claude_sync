...
async function syncProject(projectName, readme, featureCount, mapping) {
  ...
  // Helper to try updating/creating with optional properties, falling back without them
  async function tryWithOptionalProperties(fn) {
    const optionalProps = {
      'Feature Count': { number: featureCount },
      'Last Synced': { date: { start: new Date().toISOString() } }
    };

    try {
      // Try with optional properties
      return await fn({ ...properties, ...optionalProps });
    } catch (error) {
      if (error.code === 'validation_error' && error.message.includes('not a property that exists')) {
        // Retry without optional properties
  ...
  if (pageId) {
    ...
    await tryWithOptionalProperties(async (props) => {
      await notion.pages.update({
        page_id: pageId,
        properties: props
      });
    ...
    const page = await tryWithOptionalProperties(async (props) => {
